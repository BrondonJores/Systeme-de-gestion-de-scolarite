...
<body>
    ...
    <div class='dis2'> 
        <section class='table_header2'><h3>DEPOT DES DEVOIRS</h3></section>
            <section class="contain" id="con">
                <form action="devoir.php" method="post" enctype ="multipart/form-data">
                    <div class="title">
                        <h3>REMPLIR SOIGNEUSEMENT LE FORMULAIRE SUIVANT TOUTE ERREUR POURRAIS EMPECHER LE TRAITEMENT DE VOTRE DEVOIR</h3>
                    </div>
                    ...
                    <div class="input-field">
						...
						<?php
							$quer = "SELECT *,ue.Id_UE,Nom_UE, eval.Id_UE, eval.Id_type, typ.Id_type from ue, evaluation as eval, typeeval as typ where((eval.Id_UE = ue.Id_UE) && (eval.Id_type = typ.Id_type))";
							$ex = mysqli_query($con, $quer);
							if($ex){
								foreach ($ex as $value) :
                    ...
                    <div class="input-field">
                        <p>Intitulé du devoir :</p>
						<input type="text" name="rqi" id="rqi">
                    </div>

                    <div class="input-field">
                        <p>Téleverser votre devoir :</p>
    ...
    <?php 
        ...
        if(!empty($mat) && !empty($ideval) && !empty($rqi)){
            $targetDirRq = "Documents/Devoir/";
            ...
            if((move_uploaded_file($_FILES["rq"]["tmp_name"], $targetFileRq))){
                if(!empty($mat) && !empty($ideval)){
                    $query1 = "SELECT * FROM devoir";
                    $sql1=mysqli_query($con, $query1);
                    $num = mysqli_num_rows($sql1);
                    $id=$num+1;
                    $targetFileRq = $mat.$ideval.basename($_FILES["rq"]["name"]);
                    $query = "INSERT INTO devoir VALUES ($id,{$ideval},'{$mat}','{$targetFileRq}', '{$rqi}')";
                    $sql=mysqli_query($con, $query);
                    if($sql){
                            echo"<script>alert('Devoir déposée avec succès')</script>";
