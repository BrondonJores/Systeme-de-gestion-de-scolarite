    ...
    <header class="header">
        ...
        <nav class="navbar">
            ...
            <a href="indexprof.php" class="fas fa-home"></a>
                ...
                <?php
                        ...
                        if($sql){
                            foreach($sql as $v):
                                ...
                                echo"</select>";
                                echo"<p>Note</p> <input type='number' name='note' max='20' min='0' value='0'>";
                                echo"<input type='text'name='entry' id='entry' class='val' value='{$mate}'>";
                                echo"</div>";
                                echo"<button id='btnAdd''>Modifier</button>";
                                echo"</form>";
                                echo"</div>";
                                echo"</section></div>"; 
                            endforeach;   
                ...
                <?php
                    if((isset($_POST['entry'])) && (isset($_POST['note'])) && (isset($_POST['type'])) && (isset($_POST['ue']))){
                        $mate = $_POST['entry'];
                        $type = $_POST['type'];
                        $ue = $_POST['ue'];
                        $note = $_POST['note'];
                    }
                    if((!empty($mate)) && (!empty($type)) && (!empty($ue)) && (!empty($note))){
                        ...
                        foreach($sqle1 as $gd){
                            $id_eval = $gd['Id_eval'];
                            $quer = "UPDATE participer set note = {$note} where ((Matricule like '{$mate}') && (Id_eval = $id_eval))";
