    ...
    <div class='dis2'> 
        <section class='table_header2'><h3>BIENVENUE SUR ProfEtud </h3></section>
            <section class="contain" id="con">
                <?php
                        ...
                        if($sql){
                            foreach($sql as $v):
                                ...
                                echo"<h2>CARTE ETUDIANT A <span>UYI</span></h2>";
                                echo"<p>Matricule : <span>{$v['Matricule']}</span></p>";
                                echo"<p>Nom : <span>{$v['Nom']}</span></p>";
                                echo"<p>Télephone : <span>{$v['Telephone']}</span></p>";
                                echo"<p>Sexe : <span>{$v['Sexe']}</span></p>";
                ...
                ?>    
                    <div class='dash'>
                        <h2>Déposer une requête <a href="requete.php" class='fas fa-arrow-right'></a></h2>
                        <div class='containt'>
                            <h2>Requête déjà déposée</h2>
                            <?php
                                $query = "SELECT * FROM reqetrep where Matricule like '{$mate}'";
                                $sql = mysqli_query($con,$query);
                                if($sql){
                                    foreach($sql as $v):
                        ...
                        <h2>Déposer un devoir <a href='requete.php' class='fas fa-arrow-right'></a></h2>
                        <div class='containt'>
                            <h2>devoir déjà déposé</h2>
                            <?php
                                $query = "SELECT * FROM devoir where Matricule like '{$mate}'";
                                $sql = mysqli_query($con,$query);
                                if($sql){
                                    foreach($sql as $v):
                        ...
                        <h2>Consulter ses notes <a href='requete.php' class='fas fa-arrow-right'></a></h2>
                </div>
            </section>
